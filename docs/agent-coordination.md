...
- Run in hands-off pipeline mode.
- Assign visible AO agents for engineer, reviewer, and QA.
- Keep at least one agent actively working while work is pending.
- Poll active agents every 2 minutes.
- Hand work off immediately from engineer to reviewer to QA to merge.
- Merge automatically when CI is green, review passes, and QA passes.
- Clean up merged or inactive agents immediately.
- Do not wait for the user between normal handoffs.
- Notify only when a PR is opened, CI fails, a merge conflict appears, a human decision is required, work is ready to merge, or work is merged.
- If no blocker exists, continue automatically.
- If an agent is idle or blocking, reassign or replace it quickly.
